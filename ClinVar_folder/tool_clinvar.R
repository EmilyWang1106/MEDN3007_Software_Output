...
master.table<-fread("Master_table.csv")
...
duckdb::duckdb_register(con,"Master_table",master.table)
...
log_debug('master: {dbGetQuery(con,"select count(*) FROM master_table;")}')
res3<-dbExecute(con,"create or replace view master_joined as select distinct * FROM master_table left join asg using (uniprot,aa_pos,aa_alt);")
log_debug('master_joined: {dbGetQuery(con,"select count(*) FROM master_joined;")}')
res4<-dbExecute(con,"copy master_joined to 'master_joined.csv'(format csv,header);")
